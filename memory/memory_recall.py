...
class EnhancedMemoryRecall:
    ...
    def __init__(self, memory_system: "MemorySystem"):
        ...
        self.recall_strategies = {
            "semantic": 0.40,          # 语义相似度
            "keyword": 0.15,           # 关键词匹配
            "associative": 0.15,       # 联想记忆
            "temporal": 0.03,          # 时间关联
            "strength": 0.02,          # 记忆强度
            "tag_association": 0.15,   # 标签联想召回 (Phase 5)
            "cross_domain": 0.10,      # 跨域联想召回 (Phase 5)
        }

        # Phase 5: 标签联想召回组件（延迟初始化）
        self._tag_recall = None
        self._cross_domain_recall = None
    ...
    async def recall_all_relevant_memories(
        ...
        try:
            ...
            tasks = [
                ...
                self._tag_association_recall(query, group_id),    # Phase 5: 标签联想
                self._cross_domain_recall(query, group_id),       # Phase 5: 跨域联想
            ]
    ...
    async def _tag_association_recall(
        self, query: str, group_id: str = ""
    ) -> list[MemoryRecallResult]:
        """
        基于标签图谱的联想召回 (Phase 5)

        从查询文本中提取标签，在标签图中查找联想标签，
        并返回关联的记忆。
        """
        try:
            # 延迟初始化标签联想组件
            if self._tag_recall is None:
                self._tag_recall = await self._init_tag_recall()
            ...
            # 执行标签联想召回
            tag_results = self._tag_recall.recall(
                query=query,
                top_k=10,
                include_path=True,
            )
            ...
            for tag_result in tag_results:
                # 查找与标签相关的记忆
                related_memories = self._find_memories_by_tag(
                    tag_result.tag.name, group_id
                )
                for memory in related_memories:
                    ...
                    if concept:
                        relevance = tag_result.score * self.recall_strategies["tag_association"]
                        results.append(
                            MemoryRecallResult(
                                memory=memory.content,
                                relevance_score=relevance,
                                memory_type="tag_association",
                                concept_id=memory.concept_id,
                                metadata={
                                    "memory_id": memory.id,
                                    "concept_name": concept.name,
                                    "matched_tag": tag_result.tag.name,
                                    "联想路径": tag_result.path,
                                    "strategy": tag_result.source,
                                    "group_id": group_id,
                                },
                            )
                        )

            logger.debug(f"标签联想召回完成，找到 {len(results)} 条相关记忆")
            return results

        except Exception as e:
            logger.debug(f"标签联想召回跳过: {e}")
            return []

    async def _cross_domain_recall(
        self, query: str, group_id: str = ""
    ) -> list[MemoryRecallResult]:
        """
        跨域联想召回 (Phase 5)
        ...
        try:
            # 延迟初始化跨域召回组件
            if self._cross_domain_recall is None:
                self._cross_domain_recall = await self._init_cross_domain_recall()
            ...
            # 尝试货品→人群跨域
            try:
                commodity_results = self._cross_domain_recall.commodity_to_crowd(
                    keywords, top_k=5
                )
                for cr in commodity_results:
                    related_memories = self._find_memories_by_tag(
                        cr.tag.name, group_id
                    )
                    for memory in related_memories:
                        concept = self.memory_system.memory_graph.concepts.get(
                            memory.concept_id
                        ...
                        if concept:
                            relevance = cr.score * self.recall_strategies["cross_domain"]
                            results.append(
                                MemoryRecallResult(
                                    memory=memory.content,
                                    relevance_score=relevance,
                                    memory_type="cross_domain",
                                    concept_id=memory.concept_id,
                                    metadata={
                                        "memory_id": memory.id,
                                        "concept_name": concept.name,
                                        "target_tag": cr.tag.name,
                                        "source_domain": cr.source_domain,
                                        "target_domain": cr.domain,
                                        "group_id": group_id,
                                    },
                                )
                            )
            ...
            logger.debug(f"跨域联想召回完成，找到 {len(results)} 条相关记忆")
            return results

        except Exception as e:
            logger.debug(f"跨域联想召回跳过: {e}")
            return []

    # ── 辅助方法 ──────────────────────────────────────────────────

    async def _init_tag_recall(self):
        """延迟初始化标签联想召回组件"""
        try:
            from intelligence.recall.tag_association_recall import TagAssociationRecall
            ...
            return TagAssociationRecall(tag_graph, embedding_service)
        except Exception as e:
            logger.debug(f"标签联想召回初始化失败: {e}")
            return None

    async def _init_cross_domain_recall(self):
        """延迟初始化跨域召回组件"""
        try:
            from intelligence.recall.cross_domain_recall import CrossDomainRecall
            from infrastructure.tag_embedding import TagEmbeddingService
            from core.tag_graph import TagAssociationGraph
            ...
            return CrossDomainRecall(tag_graph, embedding_service)
        except Exception as e:
            logger.debug(f"跨域召回初始化失败: {e}")
            return None

    def _find_memories_by_tag(
        self, tag_name: str, group_id: str = ""
    ) -> list:
        """根据标签名称查找相关记忆"""
        memories = []
        for memory in self.memory_system.memory_graph.memories.values():
            ...
            # 检查记忆内容是否包含标签关键词
            if tag_name in memory.content:
                memories.append(memory)
            elif tag_name in memory.tags:
                memories.append(memory)

        return memories[:5]  # 限制数量
