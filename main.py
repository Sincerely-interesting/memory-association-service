...
try:
    ...
    from .intelligence.tag_system import (
        ...
        CentroidCalculator, AssociationEngine,
    )
...
class MemoraConnectPlugin(Star):
        ...
        try:
            ...
            try:
                ...
                if TAG_SYSTEM_AVAILABLE:
                    try:
                        tag_cfg = (self.memory_system.memory_config or {}).get(
                            "tag_system_enabled", True
                        )
                        if tag_cfg:
                            data_dir = StarTools.get_data_dir() / "memora_connect"
                            db_path = str(data_dir / "tag_system.db")
                            self.tag_gateway = TagAPIGateway(
                                db_path=db_path, auto_initialize=True
                            )
                            self.tag_graph = self.tag_gateway.graph
                            self.tag_embedding = self.tag_gateway.embedding_service
                            logger.info("标签联想系统初始化完成")
                    except Exception as tag_e:
                        logger.warning(f"标签系统初始化跳过: {tag_e}")
    ...
    @memory.command("标签查询")
    async def tag_search(self, event: AstrMessageEvent, keyword: str = ""):
        """查询标签"""
        if not TAG_SYSTEM_AVAILABLE or not self.tag_gateway:
            yield event.plain_result("标签系统未启用")
            return

        if not keyword:
            # 显示统计信息
            stats = self.tag_gateway.get_stats()
            if stats.get("ok"):
                data = stats["data"]
                graph_stats = data.get("graph", {})
                response = (
                    f"📊 标签系统统计:\n"
                    f"  总标签数: {graph_stats.get('total_tags', 0)}\n"
                    f"  总联想数: {graph_stats.get('total_associations', 0)}\n"
                    f"  货品域: {graph_stats.get('tags_by_domain', {}).get('commodity', 0)}\n"
                    f"  人群域: {graph_stats.get('tags_by_domain', {}).get('crowd', 0)}\n"
                    f"  平台域: {graph_stats.get('tags_by_domain', {}).get('platform', 0)}"
                )
            else:
                response = "获取统计信息失败"
            yield event.plain_result(response)
        ...
        # 搜索标签
        tags = self.tag_gateway.graph.tags.values()
        results = [t for t in tags if keyword in t.name]
        if results:
            response = f"🔍 找到 {len(results)} 个匹配标签:\n"
            for tag in results[:10]:
                response += f"  • {tag.name} ({tag.domain}, L{tag.level})\n"
        else:
            response = f"未找到包含 '{keyword}' 的标签"
        yield event.plain_result(response)

    @memory.command("联想")
    async def tag_associate(self, event: AstrMessageEvent, keyword: str = ""):
        """标签联想查询"""
        if not TAG_SYSTEM_AVAILABLE or not self.tag_gateway:
            yield event.plain_result("标签系统未启用")
            return
        ...
        if not keyword:
            yield event.plain_result("请提供查询关键词，例如: /记忆 联想 精华")
            return

        # 单域联想
        results = self.tag_gateway.associate_single({
            "tags": [keyword],
            "top_k": 5,
        })

        if results.get("ok") and results["data"].get("results"):
            response = f"🔗 '{keyword}' 的联想结果:\n"
            for r in results["data"]["results"]:
                tag = r.get("tag_name", "")
                domain = r.get("tag_domain", "")
                score = r.get("score", 0)
                response += f"  • {tag} ({domain}) - 相关度: {score:.2f}\n"
        else:
            response = f"未找到 '{keyword}' 的联想结果"
        yield event.plain_result(response)

    @memory.command("5C匹配")
    async def tag_5c_match(self, event: AstrMessageEvent, args: str = ""):
        """5C法则匹配"""
        if not TAG_SYSTEM_AVAILABLE or not self.tag_gateway:
            yield event.plain_result("标签系统未启用")
            return
        ...
        if not args:
            response = (
                "📋 5C法则匹配\n"
                "格式: /记忆 5C匹配 内容:测评 渠道:抖音 人群:Z世代 货品:精华\n"
                "示例: /记忆 5C匹配 货品:精华"
            )
            yield event.plain_result(response)
            return
        ...
        # 解析参数
        partial_5c = {"content": None, "channel": None, "crowd": None, "commodity": None}
        for part in args.split():
            if ":" in part:
                key, val = part.split(":", 1)
                if key in partial_5c:
                    partial_5c[key] = [val]

        results = self.tag_gateway.associate_5c(partial_5c)

        if results.get("ok") and results["data"].get("results"):
            response = "🎯 5C匹配结果:\n"
            for i, r in enumerate(results["data"]["results"][:3], 1):
                conv = r.get("conversion_score", 0)
                conf = r.get("confidence", 0)
                response += f"  {i}. 转化分: {conv:.2f}, 置信度: {conf:.2f}\n"
        else:
            response = "未找到匹配结果"
        yield event.plain_result(response)
