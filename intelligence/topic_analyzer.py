...
class TopicAnalyzer:
    ...
    def _extract_session_image(self, session) -> str | None:
        """从 session 的消息列表中提取第一张图片 URL"""
    ...
    def _build_prompt(
        ...
        # 1. 新消息（带序号）
        has_images = any(msg.get("image_url") for msg in messages)
        if has_images:
            parts.append("新消息（标注了 [图片: URL] 的消息包含图片，请在分析时考虑图片上下文）:")
        else:
            parts.append("新消息:")
        for i, msg in enumerate(messages):
            ...
            image_tag = f" [图片: {msg['image_url']}]" if msg.get("image_url") else ""
            parts.append(f"[{i}] [{time_str}] {sender}: {content}{image_tag}")
        ...
        if memory_data and isinstance(memory_data, dict):
            try:
                ...
                if content:
                    ...
                    # 标记待同步到 LanceDB（多模态）
                    if hasattr(self.memory_system, '_pending_lance_sync'):
                        self.memory_system._pending_lance_sync.add(memory_id)
                        # 从 session 消息中提取图片 URL，关联到记忆
                        session_image = self._extract_session_image(session)
                        if session_image:
                            if not hasattr(self.memory_system, '_memory_image_urls'):
                                self.memory_system._memory_image_urls = {}
                            self.memory_system._memory_image_urls[memory_id] = session_image
