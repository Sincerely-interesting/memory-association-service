...
class MemorySystem:
    ...
    def __init__(self, context: Context, config=None, data_dir=None):
        ...
        # 多模态记忆（可选，由 initialize_multimodal 启用）
        self._multimodal_encoder = None
        self._lance_store = None
        self._multimodal_enabled = config.get("enable_multimodal", False) if config else False
        self._pending_lance_sync: set = set()  # 待同步到 LanceDB 的 memory_id
        self._memory_image_urls: dict = {}     # memory_id → image_url 映射
        self._multimodal_config = {
            "lancedb_path": config.get("lancedb_path", "data/lancedb") if config else "data/lancedb",
            "clip_model": config.get("clip_model", "openai/clip-vit-base-patch32") if config else "openai/clip-vit-base-patch32",
            "text_weight": config.get("multimodal_text_weight", 0.5) if config else 0.5,
            "image_weight": 1.0 - (config.get("multimodal_text_weight", 0.5) if config else 0.5),
        }
    ...
    async def initialize_multimodal(self):
        """初始化多模态记忆能力（可选）

        成功：启用 CLIP 编码器 + LanceDB 向量存储
        失败：降级为纯文本模式，不影响原有功能
        """
        if not self._multimodal_enabled:
            logger.info("多模态记忆未启用")
            return
        ...
        try:
            ...
            self._multimodal_encoder = MultimodalEncoder(
                model_name=self._multimodal_config["clip_model"]
            )

            # 计算 LanceDB 绝对路径
            lancedb_path = self._multimodal_config["lancedb_path"]
            if not os.path.isabs(lancedb_path):
                lancedb_path = str(
                    os.path.dirname(self.db_path) + "/" + lancedb_path
                )

            self._lance_store = LanceDBVectorStore(db_path=lancedb_path)
            success = await self._lance_store.initialize()

            if success:
                logger.info(
                    f"多模态记忆初始化成功 | CLIP={self._multimodal_config['clip_model']} | "
                    f"LanceDB={lancedb_path}"
                )
            else:
                logger.warning("LanceDB 初始化失败，降级为纯文本模式")
                self._lance_store = None

        except Exception as e:
            logger.warning(f"多模态初始化失败，降级为纯文本模式: {e}")
            self._multimodal_encoder = None
            self._lance_store = None

    def _extract_image_from_message(self, message) -> str | None:
        """从 AstrBot 消息中提取图片 URL

        Returns:
            图片 URL 或 None
        """
        try:
            # AstrBot 消息格式：message 是消息段列表
            segments = getattr(message, "message", message)
    ...
    async def _sync_memory_to_lance(self, memory, event: str = "upsert"):
        """将记忆同步到 LanceDB

        Args:
            memory: Memory 对象
            event: "upsert" 或 "delete"
        """
        if not self._lance_store or not self._multimodal_encoder:
            return

        try:
            if event == "delete":
                await self._lance_store.delete_memory_vector(memory.id)
                return

            # 编码文本
            text_content = f"{memory.content} {memory.details or ''}"
            text_vector = self._multimodal_encoder.encode_text(text_content)

            # 获取图片 URL 并编码（如果有）
            image_vector = None
            image_url = None
            if hasattr(self, '_memory_image_urls') and memory.id in self._memory_image_urls:
                image_url = self._memory_image_urls.pop(memory.id)
                if image_url and self._multimodal_encoder:
                    try:
                        image_vector = self._multimodal_encoder.encode_image(image_url)
                        # 检查是否为零向量（编码失败）
                        if all(v == 0.0 for v in image_vector):
                            image_vector = None
                            image_url = None
                    except Exception as e:
                        self._debug_log(f"图片编码失败 {image_url}: {e}", "warning")
                        image_vector = None
                        image_url = None

            modality = "multimodal" if image_vector else "text"

            # 获取概念名称
            concept_name = ""
            if memory.concept_id:
                concept = self.memory_graph.concepts.get(memory.concept_id)
                if concept:
                    concept_name = concept.name

            await self._lance_store.upsert_memory_vector(
                memory_id=memory.id,
                ...
                text_weight=self._multimodal_config["text_weight"],
                image_weight=self._multimodal_config["image_weight"],
            )

        except Exception as e:
            self._debug_log(f"LanceDB 同步失败 memory_id={memory.id}: {e}", "warning")

    async def _sync_memories_batch_to_lance(self, memories: list):
        """批量同步记忆到 LanceDB（启动补齐用）"""
        if not self._lance_store or not self._multimodal_encoder:
            return
        ...
        for memory in memories:
            try:
                await self._sync_memory_to_lance(memory, "upsert")
                synced += 1
            except Exception:
                errors += 1

        if synced > 0:
            logger.info(f"LanceDB 批量同步完成: {synced} 成功, {errors} 失败")
    ...
    async def save_memory_state(self, group_id: str = ""):
        ...
        try:
            ...
            try:
                ...
                # 异步同步待处理的记忆到 LanceDB（多模态）
                if self._pending_lance_sync and self._lance_store:
                    pending_ids = list(self._pending_lance_sync)
                    self._pending_lance_sync.clear()
                    pending_memories = [
                        self.memory_graph.memories[mid]
                        for mid in pending_ids
                        if mid in self.memory_graph.memories
                    ]
                    if pending_memories:
                        self._create_managed_task(
                            self._sync_memories_batch_to_lance(pending_memories)
                        )
    ...
    async def delete_memory_by_id(self, memory_id: str, group_id: str = "") -> bool:
        try:
            ...
            # 同步删除 LanceDB 向量
            if self._lance_store:
                self._create_managed_task(
                    self._sync_memory_to_lance(
                        type("DeletedMemory", (), {
                            "id": memory_id,
                            "group_id": group_id,
                        })(),
        ...
        try:
            ...
            for theme in themes:
                ...
                if memory_content:
                    ...
                    self._pending_lance_sync.add(memory_id)
    ...
    async def process_message_optimized(
        ...
        try:
            ...
            # 提取图片 URL（多模态记忆）
            image_url = self._extract_image_from_message(event)
            ...
            await self.topic_analyzer.add_message(
                message, sender_id, sender_name, group_id, umo=umo, image_url=image_url
            )
